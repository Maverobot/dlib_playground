...
int main() {
  // Here we declare that our samples will be 1 dimensional column vectors.  In
  // general, you can use N dimensional vectors as inputs to the krls object.
  // But here we only have 1 dimension to make the example simple.  (Note that
  // if you don't know the dimensionality of your vectors at compile time you
  // can change the first number to a 0 and then set the size at runtime)
  typedef matrix<double, 1, 1> sample_type;

  // Now we are making a typedef for the kind of kernel we want to use.  I
  // picked the radial basis kernel because it only has one parameter and
  // generally gives good results without much fiddling.
  typedef radial_basis_kernel<sample_type> kernel_type;

  // Here we declare an instance of the krls object.  The first argument to the
  // constructor is the kernel we wish to use.  The second is a parameter that
  // determines the numerical accuracy with which the object will perform part
  // of the regression algorithm.  Generally smaller values give better results
  // but cause the algorithm to run slower.  You just have to play with it to
  // decide what balance of speed and accuracy is right for your problem. Here
  // we have set it to 0.001.
  ...
  // now we output the value of the sinc function for a few test points as well
  // as the value predicted by krls object.
  m(0) = 2.5;
  cout << sinc(m(0)) << "   " << test(m) << endl;
  m(0) = 0.1;
  cout << sinc(m(0)) << "   " << test(m) << endl;
  m(0) = -4;
  cout << sinc(m(0)) << "   " << test(m) << endl;
  ...
  // is serializable. So for example, you can save the test object to disk and
